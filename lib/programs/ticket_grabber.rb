class TicketGrabber
	...
	def perform
		...
		db.exec("SELECT issues.project_id, issues.id, issues.created_on, users.lastname, issues.subject, issues.description FROM issues, users WHERE issues.assigned_to_id = users.id AND users.lastname IN (#{last_names.join(',')})").each { |t| RedmineTicket.create_or_update(t) }
		rescue Exception => e
