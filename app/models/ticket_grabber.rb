class TicketGrabber
	...
	def perform
		...
		db.exec("SELECT status.name as ticket_status, authors.firstname as author_first, authors.lastname as author_last, issues.project_id, issues.id, issues.created_on, users.lastname, issues.subject, issues.description FROM issues, users, users as authors, issue_statuses as status WHERE status.id = issues.status_id AND issues.author_id = authors.id AND issues.assigned_to_id = users.id AND users.lastname IN (#{last_names.join(',')})").each { |t| RedmineTicket.create_or_update(t) }
		rescue Exception => e
			`say "#{e.message}"`
	end	
end
